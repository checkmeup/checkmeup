...
**Needs a ping-model design decision before implementation** (add to [decision backlog](../decisions/backlog.md)): detecting a stuck run requires a start-of-run signal distinct from today's single completion ping. The natural shape is a new endpoint (e.g. `GET /ping/{token}/start`) that's purely additive — jobs that never call it keep exactly today's behavior (single ping = success, EP-02 unaffected) — but the exact endpoint shape, and how "in progress" state is stored (new column on `cron_monitors` vs. a new table), needs a real decision before US-3401 starts. [EP-35](ep-35-overlap-detection.md) (overlap detection) depends on this same start-ping signal, so the decision should cover both epics' needs at once.
