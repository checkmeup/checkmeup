...
- [ ] Field: domain (e.g. `example.com` — apex domain only, same validation pattern as EP-04's `hostname` field)
- [ ] First check runs immediately on creation
- [ ] Shows registrar, expiry date, and days remaining after first check
- [ ] Counts toward the org's aggregate monitor limit, enforced the same way as cron/uptime/SSL creation (ADR-019)
...
- [ ] Looked up via RDAP (RFC 9082/9083 — structured JSON, no text parsing) where the TLD's registry supports it
- [ ] Falls back to WHOIS text parsing for TLDs without RDAP support
- [ ] Checked once per day (same cadence as EP-04's SSL check)
- [ ] Records: registrar, expiry date, days remaining, valid/error flag
- [ ] Lookup failure (registry timeout, unsupported TLD, unparsable response) recorded as an error state — never reported as "expired" when the real status is unknown
...
- [ ] Alerts sent at 30 days, 14 days, and 7 days before expiry — one alert per threshold, same pattern as EP-04 ([ADR-016](../decisions/016-alert-debounce.md))
- [ ] Alert sent immediately if the domain is already expired or in a registry hold/pending-delete status (when that status is exposed by RDAP)
- [ ] No repeated alerts at the same threshold
...
- [ ] "Expiring soon" shown when ≤ 30 days remaining, same threshold as EP-04
- [ ] Detail: registrar, expiry date, days remaining, last checked time, error message if any
...
- [ ] Domain is editable only by deleting and recreating, same as EP-04's hostname field
