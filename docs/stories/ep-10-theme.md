...
The app ships dark-only today ([`docs/reference/design.md`](../reference/design.md) neutrals are dark-first). Add a light palette and a switch so users can pick the appearance they prefer.
...
- [x] Light-mode values defined for every neutral token in `docs/reference/design.md` (`--bg`, `--surface`, `--surface-raised`, `--border`, `--text-muted`, `--text-dim`, `--text`, `--text-strong`)
...
\* The **public status page** turned out to be a separate, server-rendered Go template (`status_public.go`) with its own fixed light palette, hardcoded independently of `docs/reference/design.md`'s tokens — not a dark page that needed a light option. Giving it an actual dark variant would mean designing a second status-page theme from scratch, which is more than "switch my dashboard" implied. Descoped; track separately if there's real demand for a dark status page.
