...
All 3 stories are done in code. The only remaining step is LemonSqueezy dashboard setup (3 more variants, one annual per paid plan, alongside the 3 monthly ones) — see [`docs/reference/billing-setup.md`](../reference/billing-setup.md). That's account/business setup only the account holder can do, not engineering work.
