...
| Status page "hide branding" toggle ([ADR-035](035-status-page-hide-branding.md)) | — | ✓ | ✓ | ✓ |
...
- **Downgrade**: nothing is ever deleted. Behavior differs by resource type:
  ...
  - **Status page "hide branding" toggle**: a boolean, not a count, so `billing.EnforceHideBrandingLimit` (`internal/billing/enforce.go`) simply clears `hide_branding` back to `false` on every one of the org's status pages when the new plan doesn't allow it — same idempotent, unconditional-on-every-webhook shape as the count-based enforcers above.
...
Downgrade enforcement (`internal/billing/enforce.go`) is separate from the creation-time checks above: `EnforceMonitorLimit`/`EnforceNotificationChannelLimit`/`EnforceHideBrandingLimit` are called from the Paddle webhook handler (`internal/handler/billing.go`'s `Webhook`, right after `UpdateOrgPlan`). The first two pause/disable the newest-created active resources beyond the new limit, using `ListActiveMonitorsForOrg` (a union across all 5 monitor tables, newest-first) and `ListNotificationChannels` filtered to enabled; `EnforceHideBrandingLimit` clears `status_pages.hide_branding` for the org via `ClearHideBrandingForOrg`, no ordering to worry about since it's a boolean, not a count. The 5 `Resume<Type>Monitor` handlers and `UpdateNotificationChannel`'s re-enable path call `billing.CheckMonitorLimit`/`billing.CheckNotificationChannelLimit` again, against `CountActiveMonitorsForOrg`/`CountEnabledNotificationChannelsForOrg` (active/enabled-only, unlike the creation-time counts which include paused/disabled resources) — this is what blocks reactivating past the limit. `status_pages.go`'s `UpdateStatusPage` does the equivalent check inline for `hide_branding` (`billing.GetLimits(plan).HideBrandingAllowed`), since there's no separate "re-enable" endpoint for it.
