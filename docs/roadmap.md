...
- [EP-36](stories/ep-36-blog-prerendering.md) — Blog prerendering for social link previews — **3 h**. Blocked on the prerendering-approach decision in [decision backlog](decisions/backlog.md) — cosmetic gap only (Slack/Twitter/LinkedIn link-preview cards show the generic site card instead of the post's own), not a functional or indexing bug, so lowest urgency in this list.
