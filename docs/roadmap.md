...
- [EP-40](stories/ep-40-monitor-form-extraction.md) — Extract shared monitor form components — **12 h**. No blocker; pure tech debt with no user-facing change, so it only earns priority when the duplicated forms are actually being edited. Flagged by `architecture-guardrails`' sibling-duplication check — five Create/Edit view pairs maintain the same form twice.
