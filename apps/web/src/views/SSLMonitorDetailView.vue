...
async function deleteMonitor() {
  actionError.value = ''
  try {
    await monitorsApi.deleteSSL(id)
    router.push({ name: 'ssl-monitors' })
  } catch (e: unknown) {
        ...
        >
          <div class="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p class="font-mono text-sm mt-1" style="color: var(--text-dim)">{{ monitor.hostname }}</p>
            </div>
            <div class="flex items-center gap-2 flex-shrink-0">
              <Button
                variant="secondary"
                size="sm"
                @click="router.push({ name: 'ssl-monitor-edit', params: { id } })"
              >
                Edit
              </Button>
          ...
          <div class="grid grid-cols-2 gap-x-8 gap-y-3 text-sm border-t pt-4" style="border-color: var(--border)">
            ...
            <div>
              <span class="block text-xs mb-0.5" style="color: var(--text-muted)">Issuer</span>
              <span style="color: var(--text)">{{ monitor.issuer ?? '—' }}</span>
            </div>
            <div>
