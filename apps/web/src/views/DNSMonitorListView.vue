...
import type { DNSMonitor } from '@/api/monitors'
import { useDNSMonitors } from '@/composables/useDNSMonitors'

const router = useRouter()
const { data, isPending: loading, error: queryError, refetch } = useDNSMonitors()
const monitors = computed<DNSMonitor[]>(() => data.value ?? [])
...
function currentValue(m: DNSMonitor): string {
  return m.lastResolvedValue ?? '—'
}
</script>
  ...
  <AppLayout>
    <div class="p-4 md:p-8 max-w-4xl mx-auto">
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-semibold" style="color: var(--text-strong)">DNS monitors</h1>
        <Button @click="router.push({ name: 'dns-monitor-create' })">
          Add monitor
        </Button>
      </div>
      ...
      >
        <p class="text-sm mb-4" style="color: var(--text-muted)">
          No DNS monitors yet. Add one to watch a record for an unexpected change (e.g. an A record pointing somewhere it shouldn't).
        </p>
        <Button @click="router.push({ name: 'dns-monitor-create' })">
      ...
      <template v-else>
        ...
        <div class="md:hidden space-y-2">
          <div
            ...
            style="background-color: var(--surface); border-color: var(--border)"
            @click="router.push({ name: 'dns-monitor-detail', params: { id: m.id } })"
          >
            ...
            <div class="flex items-center justify-between text-xs" style="color: var(--text-dim)">
              <span class="font-mono truncate max-w-[60%]">{{ m.hostname }} ({{ m.recordType }})</span>
              <span>{{ relativeTime(m.lastCheckedAt) }}</span>
            </div>
            <div class="text-xs mt-1 font-mono truncate" style="color: var(--text-muted)">
              {{ currentValue(m) }}
            </div>
          </div>
        </div>
          ...
          <table class="w-full text-sm">
            <thead>
              <tr style="background-color: var(--surface-raised); border-bottom: 1px solid var(--border)">
                ...
                <th class="text-left px-4 py-3 font-semibold text-[10.5px] uppercase tracking-wide" style="color: var(--text-muted)">Type</th>
                <th class="text-left px-4 py-3 font-semibold text-[10.5px] uppercase tracking-wide" style="color: var(--text-muted)">Current value</th>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-3 text-xs" style="color: var(--text-dim)">{{ m.recordType }}</td>
                <td class="px-4 py-3 font-mono text-xs truncate max-w-[200px]" style="color: var(--text-dim)">{{ currentValue(m) }}</td>
                <td class="px-4 py-3">
                  <span
                    class="inline-flex items-center gap-1.5 text-xs font-medium"
                    :style="{ color: statusColors[m.status] ?? 'var(--text-muted)' }"
