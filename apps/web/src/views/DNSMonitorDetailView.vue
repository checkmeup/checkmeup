...
const { data: detail, isPending: loading, error: queryError, refetch } = useDNSMonitor(id)
...
async function togglePause() {
  ...
  try {
    if (detail.value.monitor.status === 'paused') {
      await monitorsApi.resumeDns(id)
    } else {
      await monitorsApi.pauseDns(id)
    }
    await refetch()
  } catch (e: unknown) {
...
async function deleteMonitor() {
  actionError.value = ''
  try {
    await monitorsApi.deleteDns(id)
    router.push({ name: 'dns-monitors' })
  } catch (e: unknown) {
    ...
    <div class="p-8 max-w-4xl mx-auto">
      <div class="flex items-center gap-3 mb-6 flex-wrap">
        <button
          ...
          @click="router.push({ name: 'dns-monitors' })"
        >
          ← Back
        </button>
      ...
      <template v-else-if="detail">
        ...
        >
          <div class="space-y-1 min-w-0">
            <p class="font-mono text-xs break-all" style="color: var(--text-dim)">
              {{ detail.monitor.hostname }} ({{ detail.monitor.recordType }})
            </p>
            ...
            <p class="text-xs" style="color: var(--text-muted)">
              {{ detail.monitor.baselineCaptured ? 'Baseline' : 'Expected value' }}:
              <span class="font-mono">{{ detail.monitor.expectedValue ?? '(capturing on next check)' }}</span>
            </p>
            <p v-if="detail.monitor.lastResolvedValue" class="text-xs" style="color: var(--text-muted)">
              Current value: <span class="font-mono">{{ detail.monitor.lastResolvedValue }}</span>
            </p>
          </div>
          <div class="flex items-center gap-2 flex-shrink-0">
            <Button
              variant="secondary"
              size="sm"
              @click="router.push({ name: 'dns-monitor-edit', params: { id } })"
            >
              Edit
            </Button>
        ...
        <div
          ...
          style="background-color: var(--surface); border-color: var(--border)"
        >
          <h2 class="text-sm font-medium mb-3" style="color: var(--text-strong)">Lookup time — last 24h</h2>
          <svg
          ...
          <p class="text-xs mt-1" style="color: var(--text-muted)">
            Red dots = failed checks (mismatch or lookup error). Y axis = lookup time (max {{ Math.max(...(detail.chartData.map(c => c.responseTimeMs)), 0) }}ms).
          </p>
        </div>
          ...
          <table v-else class="w-full text-sm">
            <thead>
              <tr style="border-bottom: 1px solid var(--border)">
                ...
                <th class="text-left px-5 py-2 font-medium text-xs" style="color: var(--text-muted)">Lookup time</th>
                <th class="text-left px-5 py-2 font-medium text-xs" style="color: var(--text-muted)">Resolved value</th>
                <th class="text-left px-5 py-2 font-medium text-xs" style="color: var(--text-muted)">Error</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="px-5 py-2.5 font-mono text-xs" style="color: var(--text-dim)">{{ c.resolvedValue ?? '—' }}</td>
