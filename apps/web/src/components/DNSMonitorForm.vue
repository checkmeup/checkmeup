...
import {
  ...
  intervalOptionsFor,
  recordTypeOptions,
  type DNSMonitorFormState,
} from '@/lib/dnsMonitorForm'

const form = defineModel<DNSMonitorFormState>({ required: true })
...
// What "expected value" means differs by screen: on create it explains
// baseline auto-detection, on edit it explains re-arming that baseline. The
// wording is genuinely different, not a string swap, so it comes from a slot.
defineSlots<{ expectedValueHelp?: () => unknown }>()
</script>
...
<template>
  <div class="space-y-5">
    <div>
      ...
      <Input id="name" v-model="form.name" placeholder="Apex A record" class="mt-1" required />
    </div>

    <div class="grid grid-cols-3 gap-3">
      <div class="col-span-2">
        <Label for="hostname">Hostname</Label>
        <Input id="hostname" v-model="form.hostname" placeholder="example.com" class="mt-1" />
      </div>
      <div>
